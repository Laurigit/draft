...
createNewDecklist_after_changes <- function(input_new_DCIDs, removed_DCIDs, Pakka_ID_input, STG_CARDS, STG_CARDS_DIM, STG_DRAFT_CARDS, con) {
  ...
  #new pfi
  oldMaxPfi <- as.numeric(dbQ("SELECT max(Pakka_form_ID) AS Maxpfi from CARDS", con))
  new_pfi <-  oldMaxPfi + 1
